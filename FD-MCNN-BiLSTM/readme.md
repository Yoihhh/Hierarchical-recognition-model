...
概览
----
本项目搭建一个两阶段的混叠信号识别流程：数据预处理 → FD-MCNN 强信号检测 → 注意力增强弱信号 → BiLSTM 弱信号分类。当前完成了数据预处理管线和代码骨架，其余网络将后续补充。

环境要求
--------
- Python 3.9
- PyTorch 1.12.0+cu116（仅当保存为 pt 或后续训练时需要）
- TorchVision 0.13.0+cu116（未来训练可用）
- numpy、scipy（预处理必需）
